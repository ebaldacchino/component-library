...
import { DayView, type TIsoDate } from "../DatePicker";
...
interface DateRangePickerProps {
	viewedDate: TIsoDate;
	onChange: (newDates: TIsoDate[]) => void;
	minDate: TIsoDate;
	maxDate: TIsoDate;
...
export function DateRangePicker(props: DateRangePickerProps) {
	...
	const [selectedDates, setSelectedDates] = useState<TIsoDate[]>([]);
	const [viewedDate, setViewedDate] = useState(
		selectedDates[0] ?? (new Date().toISOString() as TIsoDate)
	);
				...
				<DatePickerHeader
					onMonthChange={(date) => {
						...
						const newDateIsoString = newDateObject
							.toISOString()
							.slice(0, 10) as TIsoDate;

						setViewedDate(newDateIsoString);
					}}
					viewedDate={
						secondViewedDateObject.toISOString() as TIsoDate
					}
					onChange={handleChange}
					minDate={minDate}
					maxDate={maxDate}
